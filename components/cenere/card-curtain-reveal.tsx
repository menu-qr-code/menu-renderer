...
function useCardCurtainRevealContext() {
  ...
  if (!context) throw new Error("Must be used within CardCurtainReveal")
  return context
}
...
const CardCurtainReveal = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ children, className, ...props }, ref) => {
    ...
    const handleTouchStart = React.useCallback(() => setIsMouseIn(true), [])
    const handleTouchEnd = React.useCallback(() => setTimeout(() => setIsMouseIn(false), 1200), [])
